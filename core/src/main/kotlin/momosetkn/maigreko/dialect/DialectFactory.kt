...
/**
 * Factory for creating Dialect instances using ServiceLoader.
 */
object DialectFactory {
    /**
     * Create Dialect by detecting the dialect via the provided DataSource.
     * Preferred when you only have a DataSource.
     */
    fun create(dataSource: DataSource): Dialect {
    ...
    /**
     * Create Dialect by explicitly specifying a dialect key (e.g., "postgresql", "mysql").
     */
    fun create(dialect: String): Dialect {
        val loader = ServiceLoader.load(Dialect::class.java)
        val iterator = loader.iterator()
        val dialectLower = dialect.lowercase()
        val candidates = mutableListOf<Dialect>()
        ...
        return when (candidates.size) {
            0 -> throw IllegalArgumentException("No Dialect found for $dialect")
            1 -> candidates.first()
            else -> {
                val candidateNames = candidates.map { it::class.qualifiedName }
                throw IllegalArgumentException(
                    "Multiple Dialect candidates found for dialect: $dialect -> $candidateNames"
                )
            }
        }
    }
    ...
    /**
     * Create the first available Dialect from ServiceLoader as a fallback.
     */
    fun createFirst(): Dialect {
        val loader = ServiceLoader.load(Dialect::class.java)
        val it = loader.iterator()
        if (it.hasNext()) return it.next()
        throw IllegalArgumentException("No Dialect implementation found")
    }
